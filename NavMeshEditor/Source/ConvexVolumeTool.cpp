...
void ConvexVolumeTool::handleMenu()
{
    ...
    imguiSeparator();
    imguiSlider("Shape Height", &m_boxHeight, 10.0f, 1000.0f, 10.0f);
	imguiSlider("Shape Descent", &m_boxDescent, 0.1f, 20.0f, 0.1f);
	imguiSlider("Poly Offset", &m_polyOffset, 0.0f, 10.0f, 0.1f);
	imguiSeparator();

    if (imguiCheck("Inhabited", m_inhabited, !m_visualize)) {
		m_inhabited = !m_inhabited;
    }
	imguiLabel("Area Type");
	imguiIndent();
    if (imguiCheck("Ground", PolyAreaFlags::clearInhabitedFlag(m_areaType) == PolyAreaFlags::GROUND, !m_visualize))
		m_areaType = PolyAreaFlags::GROUND;
    if (imguiCheck("Road", PolyAreaFlags::clearInhabitedFlag(m_areaType) == PolyAreaFlags::ROAD, !m_visualize))
		m_areaType = PolyAreaFlags::ROAD;
    if (imguiCheck("Forest", PolyAreaFlags::clearInhabitedFlag(m_areaType) == PolyAreaFlags::FOREST, !m_visualize))
        m_areaType = PolyAreaFlags::FOREST;
	if (m_inhabited)
        m_areaType |= PolyAreaFlags::appendInhabitedFlag(m_areaType);
	imguiUnindent();
    ...
    if (imguiButton("Clear marked areas"))
	{
		m_npts = 0;
		m_nhull = 0;
	...
	if (imguiButton("Save marked areas"))
	{
		InputGeom* geom = m_sample->getInputGeom();
...
auto ConvexVolumeTool::getConvexAreaData (int i) const
{
    InputGeom* geom = m_sample->getInputGeom();
	const auto& dat = *geom->getConvexVolume(i);
	return std::make_tuple(
		dat.vertsNum, static_cast<const float*>(dat.verts), dat.minh, dat.maxh
	);
}
...
void ConvexVolumeTool::handleClick(const float* /*s*/, const float* p, bool shift)
{
    ...
    if (m_visualize)
    {
        ...
        for (int i = 0, n = geom->getConvexVolumeCount(); i < n; ++i)
        {
            const auto dat = getConvexAreaData(i);
            int nverts = std::get<0>(dat);
            const float* verts = std::get<1>(dat);
            float hmin = std::get<2>(dat);
            float hmax = std::get<3>(dat);
            if (
                (p[1] >= hmin && p[1] <= hmax) && pointInPoly(nverts, verts, p)
            ) {
                m_volumeIdVisualization = i;
                found = true;
                break;
            }
        }
...
void ConvexVolumeTool::handleRenderOverlay(double* proj, double* model, int* view)
{
    ...
    if (m_volumeIdVisualization != INVALID_VISUAL_ID)
    {
        InputGeom* geom = m_sample->getInputGeom();
        if (!geom)
            return;
        ...
        char buf[64];
        const char* areaType;
        const char* inhabUninhab;
        uint8_t area = PolyAreaFlags::clearInhabitedFlag(m->area);
        if (area == PolyAreaFlags::GROUND) {
            areaType = "ground";
        }
        ...
        inhabUninhab = PolyAreaFlags::isInhabitedFlag(m->area) ? "inhabited" : "uninhabited";
        std::sprintf(buf, "marked area: %s, %s", areaType, inhabUninhab);
        GLdouble x, y, z;
        if (gluProject(
