...
static void drawMeshTile(
    duDebugDraw* dd, const dtNavMesh& mesh, const dtNavMeshQuery* query,
...
static void drawAverageNavmeshPolysTile(duDebugDraw* dd, const dtMeshTile* tile)
{
	...
	float vDat[DT_VERTS_PER_POLYGON * 3];
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		const dtPoly* p = &tile->polys[i];
		if (p->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)	// Skip off-mesh links.
		...
		for (int j = 0; j < n; ++j)
		{
			const float* v = &tile->verts[p->verts[j] * 3];
...
void duDebugDrawNavMeshPolyPoints(duDebugDraw* dd, const dtNavMesh& mesh, dtPolyRef ref)
{
    ...
    {
        const float* v = &tile->verts[poly->verts[j] * 3];
        duAppendCircle(dd, v[0], v[1], v[2], 1.f, duRGBA(255,0,0,255));
    }
    dd->end();
}
